...
- Next: Continue the Azure Postgres migration (dump at `~/Downloads/mindforum_20260526_150338.dump` — create Flexible Server, restore, repoint `POSTGRES_URL`, redeploy). Open follow-up PR to re-introduce the deferred admin-facilitator UI (close/mute/remove/rename) on top of creator-rooms `/admin/rooms`. Side items still pending: OpenAI monthly spend cap (defense-in-depth #2), faculty invitation for `ai-ethics-exercise`, overdue MSBAi four-week review.
